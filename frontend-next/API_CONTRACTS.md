...
| Folders       | 1         | Document organization   |
...
Update document metadata, including filename, case assignment, document type, document date, and even case title inline editing.

**Request:**

```json
{
  "filename": "updated_motion_to_dismiss.pdf",
  "case_id": 2,
...
| Field         | Type    | Required | Description                                                               |
| ------------- | ------- | -------- | ------------------------------------------------------------------------- |
| filename      | string  | No       | Rename the document (display name, not file path)                         |
| case_id       | integer | No       | Assign document to a case (null to unassign)                              |
| document_type | string  | No       | contract, pleading, evidence, correspondence, brief, motion, order, other |
| document_date | date    | No       | Document date (YYYY-MM-DD format)                                         |
| folder        | integer | No       | Assign document to a folder for organization                              |
| case          | object  | No       | Nested case update for inline case title editing                          |
| case.id       | integer | No       | Case ID (must match case_id if both provided)                             |
| case.title    | string  | No       | New case title (only updated if case_id is set)                           |
...
```json
{
  ...
  "case": {
    ...
    "title": "Updated Case Title",
    "case_number": "2024-CV-002",
    "client_name": "Johnson Corporation"
  }
}
```
...
```json
{
  "case_id": ["Invalid pk \"999\" - object does not exist."]
}
```

```json
{
  "document_type": ["\"invalid_type\" is not a valid choice."]
}
```
...
List all folders for document organization.
...
| parent_folder | integer  | Parent folder ID (null for root) |
