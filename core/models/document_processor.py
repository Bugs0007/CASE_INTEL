...
class DocumentProcessor:
    ...
    def _get_nlp(self):
        ...
        try:
            import spacy
            # en_core_web_sm is ~12 MB; we only need the sentencizer
            try:
                self._nlp = spacy.load("en_core_web_sm", disable=["ner", "tagger", "parser"])
                # Add sentencizer if the parser is disabled
                if "sentencizer" not in self._nlp.pipe_names:
                    self._nlp.add_pipe("sentencizer")
            except OSError:
                # Model not downloaded yet — use blank pipeline + sentencizer
                logger.warning(
    ...
    def chunk_text(self, text: str) -> list[str]:
        ...
        Strategy:
          ...
          2. Accumulate sentences until the chunk reaches ``chunk_size``.
          3. When starting the next chunk, backtrack ``chunk_overlap`` chars
             worth of sentences so context carries over.

        Falls back to a regex-based splitter when spaCy is unavailable.
        """
        if not text.strip():
            return []
        ...
        for sentence in sentences:
            ...
            # If this single sentence is already longer than chunk_size,
            # hard-split it rather than creating a monster chunk.
            ...
            if current_len + sent_len > self._chunk_size and current_sentences:
                ...
                # Keep overlap: trim from the front until we're under overlap limit
    ...
    def _populate_search_vectors(document_id: int) -> None:
        ...
        PostgreSQL's to_tsvector() builds an inverted index representation
        that enables fast BM25-style keyword matching.  We use 'english'
        to enable stemming (e.g. "argued" matches "argue").
        """
    ...
    def _clone_chunks_from(source_doc: Document, target_doc: Document) -> int:
        ...
        Called when an identical file (same SHA-256) is uploaded again.
        We create new DocumentChunk rows pointing to the new Document so
        the target document is fully functional, but we skip the expensive
        embedding step entirely.

        Returns the number of chunks cloned.
        """
        source_chunks = list(
    ...
    def process_document(self, document_id: int) -> Document:
        ...
        Steps:
          0. Compute SHA-256 hash → check for duplicates
             If duplicate found: clone chunks, mark completed, return early.
          1. Mark document as processing.
          2. Extract text from file.
          3. Sentence-aware chunking (spaCy).
          4. Batch embed chunks via Ollama.
          5. Persist chunk rows with bulk_create.
          6. Populate search_vector (tsvector) in one SQL UPDATE.
        ...
        try:
            ...
            if existing and existing.chunk_count:
                ...
                # Extract text so the document row is complete
            ...
            # ----------------------------------------------------------
            # Step 2: Chunk text (sentence-aware)
            # ----------------------------------------------------------
            ...
            chunk_objects = [
                DocumentChunk(
                    ...
                    embedding=embedding,
                    # search_vector is set via SQL in the next step
                )
            ...
            # ----------------------------------------------------------
            # Step 6: Populate tsvector for BM25 keyword search
            # ----------------------------------------------------------
